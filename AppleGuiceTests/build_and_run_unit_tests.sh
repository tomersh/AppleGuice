...
echo "========================="
xcodebuild -sdk iphonesimulator -workspace "${MAIN_APP_TARGET}".xcworkspace -scheme "${UNIT_TEST_TARGET}" build CONFIGURATION_BUILD_DIR="${OUTPUT_DIR}"
if [[ $? != 0 ]]; then
...
echo "==========================="
xcodebuild -sdk iphonesimulator -workspace "${MAIN_APP_TARGET}".xcworkspace -scheme "${MAIN_APP_TARGET}" build CONFIGURATION_BUILD_DIR="${OUTPUT_DIR}" XCODE_TEST_LDFLAGS="${XCODE_TEST_LDFLAGS}"
if [[ $? != 0 ]]; then
